...
import { createCheckinService } from "../../../lib/checkin-service";
...
export async function GET(request: Request) {
  ...
  if (isD1CheckinStore()) return getD1Dashboard(request);
  return getNotionDashboard();
}

async function getD1Dashboard(request: Request) {
  let checkins;
  try {
    ...
    checkins = await service.getDashboard(ownerKey);
  } catch {
  ...
  try {
    const [weeks, finances, exercises] = await Promise.all([
    ...
    return Response.json({
      ...
      checkin: latest ? {
        date: new Intl.DateTimeFormat("pt-BR", { day: "numeric", month: "long", timeZone: "America/Sao_Paulo" }).format(new Date(`${latest.checkinDate}T12:00:00-03:00`)),
        mood: latest.mood || "Não informado",
        energy: latest.energy,
        win: latest.win || "Check-in registrado.",
...
async function getNotionDashboard() {
  try {
